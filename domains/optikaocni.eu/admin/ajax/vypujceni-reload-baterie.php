...
$kola = [];
foreach ($database->baterie as $loop) {

    if ($database->vypujceni_baterie->where("baterie_id", $loop['id'])
...
foreach ($kola as $kolo) {
    ...
    <tr>
        <td><?= $kolo['cisloBaterie'] ?></td>
        <td><?= $kolo['nazev'] ?></td>
        <td style="text-align:center"><?= $kolo['kapacita'] ?></td> 
        <td style="text-align:center"><?= $kolo['info'] ?></td>
        <style>
